...
async def start_record(call : CallbackQuery, state : FSMContext):
    await call.message.edit_caption(caption="Выберите день: ", reply_markup=record_set_day_kb(datetime.today()))
...
async def set_day(call : CallbackQuery, state : FSMContext):
    ...
    await call.message.edit_media(photo=FSInputFile(filepath))
    await state.update_data(date = date)
    await state.update_data(original_message = call.message)
    await state.set_state(RecordInfo.machine)
    await call.message.edit_caption(caption="Выберите машинку: ", reply_markup=record_set_machine_kb())
...
async def laundry_pay(call : CallbackQuery, state : FSMContext):
    ...
    await call.message.edit_caption(text="Оплата проведена успешно!", reply_markup=None)
