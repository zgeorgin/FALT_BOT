...
async def set_day(call : CallbackQuery, state : FSMContext):
    ...
    await call.bot.send_photo(call.message.chat.id, photo=FSInputFile(filepath))
...
async def send_manual_time(message : Message, state : FSMContext):
    try:
        ...
        if schedule.is_time_available(data["date"], data["machine"], begin_time, end_time):
            ...
            return
        await data["original_message"].edit_text(text = "На это время нельзя записаться!!! Введите другое время", reply_markup=record_set_time_kb(schedule, data["date"], data["machine"]))
    except Exception as e:
        print(e)
        await state.set_state(RecordInfo.manual_time)
        await data["original_message"].edit_text(text="Неверный формат ввода!!! Попробуйте ещё раз", reply_markup=record_set_time_kb(schedule, data["date"], data["machine"]))
...
async def cart_view(message : Message, state : FSMContext):
    ...
    await message.edit_text(text = msg_text, reply_markup=cart_kb(data["date"]))
...
async def laundry_pay(call : CallbackQuery, state : FSMContext):
    ...
    await call.message.edit_text(text="Оплата проведена успешно!", reply_markup=None)
